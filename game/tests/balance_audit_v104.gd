...
func run():
	...
	FileAccess.open("res://../validation/v104-balance-audit.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print("BALANCE_AUDIT_COMPLETE ",report.weapons.size());quit()
