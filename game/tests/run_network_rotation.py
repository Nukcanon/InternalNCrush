...
base=[os.environ.get('GODOT','godot'),'--headless','--path',str(project),'--max-fps','60','--script','res://tests/network_rotation.gd','--']
with tempfile.TemporaryDirectory(prefix='inc-props-') as folder:
    ...
    try:
        ...
        for path in barrier.glob('*.log'):
            print(path.name,'\n'.join(line for line in path.read_text(encoding='utf-8',errors='replace').splitlines() if any(key in line for key in ['ROTATION_','ERROR','Error'])))
