...
func run():
	...
	for id in Catalog.weapons:
		...
		expect((is_equal_approx(CombatBalance.damage_at(w,5.,"head"),CombatBalance.damage_at(w,5.,"torso"))) if w.get("rocket",false) else (CombatBalance.damage_at(w,5.,"head")==300. and CombatBalance.damage_at(w,5.,"legs")==150. and CombatBalance.damage_at(w,5.,"hands")==90.) if id=="r2" else (CombatBalance.damage_at(w,5.,"head")>CombatBalance.damage_at(w,5.,"torso") and CombatBalance.damage_at(w,5.,"legs")<CombatBalance.damage_at(w,5.,"torso")),id+" zone multipliers")
