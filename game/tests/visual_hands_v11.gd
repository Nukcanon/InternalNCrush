...
func run():
	DirAccess.make_dir_recursive_absolute("res://../validation/v11-hands")
	...
	for id in ["a1","r2","e1","pistol"]:
		...
		for hand in [-1,1]:
			...
			root.get_texture().get_image().save_png("res://../validation/v11-hands/"+id+"-"+str(hand)+".png")
		p.reload_started=99.;p.reload=102.
		for i in range(6):a.visual(.016,p,100.);await process_frame
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://../validation/v11-hands/"+id+"-reload.png")
