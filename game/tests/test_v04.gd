...
func run():
	...
	for i in range(16):g.clock+=.01;AimModel.recover(p,Catalog.get_weapon("a1"),.01,g.clock)
	...
	a.input_state.z=-1
	for i in range(90):a.simulate(.016,g.clock,true)
	var moving=a.spread_angle
	a.input_state.z=0
