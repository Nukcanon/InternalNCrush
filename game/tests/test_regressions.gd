...
func run():
	...
	expect(p.primary=="c1" and p.smoke==0 and p.flash_count==3,"dead player selection and gadget composition survive respawn")
	g.clock+=1;g.handle_command(1,"slot",{"slot":2});expect(p.slot==2,"numeric gadget slot selectable")
	g.clock+=1;g.handle_command(1,"slot",{"slot":3});expect(p.slot==2 and p.gadget==1,"fourth slot cannot switch selected grenade pack")
