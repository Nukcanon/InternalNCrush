...
func run():
	...
	var character=CharacterVisual.new();target.add_child(character);character.build(1,0);character.update_pose(.016,Vector3.ZERO,false,false,true,0.,-1.,0.)
	var wall=MeshFactory.box(scene,Vector3(-20,1,2),Vector3(1.3,2.5,.3),Color("353d48"));wall.material_override.shading_mode=BaseMaterial3D.SHADING_MODE_UNSHADED
	for i in range(8):await process_frame
	await RenderingServer.frame_post_draw
	var baseline=root.get_texture().get_image();baseline.save_png("res://../validation/v113-person-base.png")
	DeploymentSilhouette.apply(target,{"owner":1,"team":0},1)
	for i in range(8):await process_frame
	await RenderingServer.frame_post_draw
	var overlay=root.get_texture().get_image();overlay.save_png("res://../validation/v113-person-overlay.png")
	var visible_delta=0.;var hidden_delta=0.
	for y in range(195,285):
		for x in range(280,360):visible_delta+=baseline.get_pixel(x,y).r-overlay.get_pixel(x,y).r
	...
	var ok=absf(visible_delta)<1. and wrong_viewer_delta<1.
