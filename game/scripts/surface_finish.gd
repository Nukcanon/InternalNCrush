...
void fragment(){
 float cloth=step(.75,UV2.x);
 float distance_fade=clamp(1.-length(fwidth(p))*160.,0.,1.);
 float weave=sin(p.x*620.)*sin(p.y*617.);
 float pore=sin(p.x*511.+p.z*379.)*sin(p.y*473.+p.z*419.);
 float detail=mix(pore*.009,weave*.026,cloth)*distance_fade;
 ALBEDO=COLOR.rgb*(1.+detail);ROUGHNESS=clamp(UV2.x+detail,.52,.96);
 METALLIC=0.;SPECULAR=mix(.25,.13,cloth);
}
"""
	human=ShaderMaterial.new();human.shader=shader;return human
