...
func _ready():
	...
	var main=VBoxContainer.new();add_child(main);var title=Label.new();title.text="경기 기록 / TAB · 처치 100 · 도움 50 · 회복 1HP당 0.3 · 설치 20 · 사망 −25";title.add_theme_font_size_override("font_size",24);main.add_child(title)
...
func refresh_scores(dt=.016):
	...
	for side in range(2):
		...
		row(box,["플레이어","병과","K","D","A","힐","설치","점수"],false,true)
		var count=0
		for i in range(players.size()):
			var p=players[i]
			if (ffa and i/16!=side) or (not ffa and p.team!=side):continue
			row(box,[(str(i+1)+". " if ffa else "")+p.nick+ ("  · 나" if p.id==game.local_id else ""),Rules.CLASSES[p.role] if game.options.classes else "—",str(p.kills),str(p.deaths),str(p.assists),str(int(p.healed)),str(int(p.get("builds",0))),str(Rules.score(p))],p.id==game.local_id,false,not p.alive);count+=1
...
func row(parent:Node,values:Array,highlight:bool,header=false,inactive=false):
	...
	for i in range(values.size()):
		...
		else:l.custom_minimum_size.x=48 if i==1 else 34;l.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
		line.add_child(l)
		if i==0 and inactive:
