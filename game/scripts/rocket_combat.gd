...
static func explode(g:Node,rocket:Dictionary,hit:Dictionary):
	...
	for id in g.players:
		...
		var splash=lerpf(30.,10.,clampf(distance/RADIUS,0.,1.))
		...
		g.damage(id,(40. if direct else 0.)+splash,owner,false,"h4",rocket.origin if direct else pos,hit.position if direct else point)
	...
	for did in g.devices.keys():
		...
		if direct or (distance<=RADIUS and g.clear_line(pos,point,exclude)):g.damage_device(did,(40. if direct else 0.)+lerpf(30.,10.,clampf(distance/RADIUS,0.,1.)),owner)
