...
static func magnification(profile:Dictionary,w:Dictionary) -> float:
	var value=float(profile.get("scope_zoom",{}).get(w.name,4.))
	return value if value in steps(w) else 4.
...
static func change(game:Node,direction:int):
	...
	var values=steps(w);var index=values.find(int(magnification(game.profile,w)))
