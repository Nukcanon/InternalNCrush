...
func build(source:CharacterVisual,pos:Vector3,push:Vector3,role:int,team:int,facing:float,_crouched:bool,previous_velocity:Vector3,_point:Vector3=Vector3.INF):
	...
	# Same exaggerated direction as native; one collider handles the Web fall.
	velocity=horizontal*3.8+previous_velocity.limit_length(4.)*.2+Vector3.UP*1.8
...
func _process(dt:float):
	...
	if age>=.9:
		...
		model.left_arm.rotation=Vector3(0,0,-1.1);model.right_arm.rotation=Vector3(0,0,1.1)
