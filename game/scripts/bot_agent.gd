...
func tick(dt:float):
	...
	if visible_target and target!=0 and game.players.has(target) and game.players[target].alive:
		...
		if action=="engage" and distance<48:
			...
			if lateral!=0.:
				var step=combat_move.normalized()*1.2;var from=a.position+Vector3.UP*.65
				var blocked=not game.ray(from,from+step,[a.get_rid()],1|4|8).is_empty()
				var floor=game.ray(a.position+step+Vector3.UP,a.position+step-Vector3.UP*1.7,[a.get_rid()],1)
				if blocked or floor.is_empty():combat_move=Vector3.ZERO
			if p.role==2 and combat_move.length_squared()<.01 and distance>18:a.input_state.crouch=true
	else:
...
static func combat_strafe(w:Dictionary,distance:float,now:float,pid:int,level:int,firing:bool) -> float:
	var scoped=float(w.get("zoom",58))<=38
	if scoped and firing and distance>12:return 0.
	var phase=fmod(now+abs(pid)*.317,2.8)
	var stationary=.9 if distance<18 else 1.35 if float(w.get("weight_kg",3.))>5. else 1.15
	if phase<stationary:return 0.
	var speed=[.42,.58,.72][clampi(level,0,2)]*clampf(float(w.get("portability",65))/75.,.55,1.)
	return speed*(1. if fmod(floor((now+abs(pid)*.317)/2.8),2.)==0 else -1.)
