	...
	if not meshes.has(key):
		...
		for face in info.faces:
			for index in face:
				...
				st.set_bones(bones);st.set_weights(weights);st.set_color(color.srgb_to_linear());st.set_uv2(Vector2(.61 if kind==0 else .91,0.))
				st.add_vertex(Vector3(p[0],p[1]-.94,p[2]))
		st.generate_normals();st.index();meshes[key]=st.commit()
	var mesh=MeshInstance3D.new();mesh.name="SculptedShirt";mesh.mesh=meshes[key];mesh.material_override=SurfaceFinish.human_material();mesh.set_meta("authored_anatomy",true);root.get_node("Hips").add_child(mesh)
