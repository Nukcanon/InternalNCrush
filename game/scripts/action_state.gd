...
static func available(game:Node,id:int,action:String) -> bool:
	...
	match action:
		...
		"gadget":
			...
			if p.role==2:return a.input_state.crouch
			if p.role==3:
		...
		"gadget_mode":return game.options.classes and p.role==4 and p.gadget!=9
		"reload":
...
static func equipment_ready(game:Node,id:int,slot:int) -> bool:
	...
	if p.role==4:
		return game.phase=="combat" and game.can_attack(p) and p.gadget_ready<=game.clock and p.gadget_count>0 and (p.smoke>0 if slot==2 else p.flash_count>0)
