extends SceneTree
class BuildClock extends Node:
	var clock=0.
const OUT="res://../validation/v119/"
...
func run():
	...
	for wid in Catalog.weapons:
		if "--quick" in OS.get_cmdline_user_args() and wid not in ["pistol","repair","remote","h4"]:continue
	...
	var carried=GadgetVisual.new();mount.add_child(carried);carried.build(3,0,true,true);await capture("carry-turret");carried.free()
	var clock_node=BuildClock.new();world.add_child(clock_node)
	mount.position=Vector3(0,-.5,-2.4)
	for kind in ["cover","turret"]:
		for team in [0,1]:
			var device=StaticBody3D.new();mount.add_child(device);CombatFX.device(device,kind,team)
			var d={"kind":kind,"team":team,"building_started":0.,"building_until":5.}
			for age in [.1,2.5,5.1]:
				clock_node.clock=age;Construction.visual(clock_node,device,d);await capture("build-%s-%s-%s"%[kind,team,age])
			device.free()
