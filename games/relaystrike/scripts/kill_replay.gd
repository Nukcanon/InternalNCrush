...
func _process(dt):
	...
	for id in models:
		...
		if id==int(event.victim) and elapsed>=FIRST_PERSON_SECONDS:
			if not fall_started:visual.animator.play("fall_back");fall_started=true;game.play_sound("hurt",Vector3.ZERO,false)
			visual.animator.seek(minf(.85,(elapsed-FIRST_PERSON_SECONDS)*1.7),true);visual.animator.pause()
	...
	if elapsed<FIRST_PERSON_SECONDS+DEATH_SECONDS:
		...
		if elapsed>=RUNUP_SECONDS:
			...
			if elapsed<FIRST_PERSON_SECONDS:
				...
				bullet.show();bullet_trail.show();bullet.position=start.lerp(end,t)
