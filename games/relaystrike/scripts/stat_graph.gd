...
func configure(kind:int,w:Dictionary,role:int,variant:int):
	...
	if kind==1 and w.kind=="gun":
		...
		match role:
			...
			1:rows=[["표식 거리",160.,200.,"160 m"],["지속 시간",4.,15.,"4초"]]
			2:rows=[["지속 시간",15.,20.,"15초"],["정지 퍼짐 감소",65.,100.,"65%"]]
			3:rows=[["내구도",[250.,500.,800.][variant],800.,str([250,500,800][variant])],["최대 설치",2.,3.,"2개"]]
			4:rows=[["연막 지속",12.,15.,"12초"],["연막 반경",5.,10.,"5 m"],["섬광 반경",15.,20.,"15 m"],["섬광 지속",2.5,5.,"2.5초"]]
			5:rows=[["즉시 회복",25.,100.,"25 HP"],["작동 거리",4.,20.,"4 m"]]
	elif kind==3:rows=[["최대 체력",100.,150.,"100 HP"],["추가 방어구",variant*25.,50.,str(variant*25)]]
	elif kind==4:
		var cooldown=[18.,30.,30.,30.,25.,20.][role]
		rows=[["재사용 대기",cooldown,40.,"%d초"%cooldown],["효과 시간",[.28,3.,4.,180.,8.,4.][role],180. if role==3 else 10.,"설치형" if role==3 else "%.2f초"%[.28,3.,4.,180.,8.,4.][role]]]
		if role==3:rows.append_array([["포탑 내구도",200.,350.,"200 → 350"],["초당 피해",20.,32.,"20 → 32"],["탐지 거리",25.,40.,"25 → 40 m"]])
	else:visible=false
	custom_minimum_size=Vector2(430,rows.size()*19+(49 if not weapon.is_empty() else 6));queue_redraw()
func _draw():
