...
func build(source:CharacterVisual,pos:Vector3,push:Vector3,role:int,team:int,facing:float,crouched:bool,velocity:Vector3,impact_point:Vector3=Vector3.INF):
	...
	for part in PARTS:
		...
		body.linear_velocity=velocity.limit_length(16.)*.9+push.limit_length(1.)*2.5;body.angular_velocity=Vector3(push.z,0,-push.x)*1.2
	...
	if impact_point.is_finite():
		...
		closest.apply_impulse(push.limit_length(1.)*minf(15.,closest.mass*1.8),(impact_point-closest.global_position).limit_length(.2))
	model.sync_deform()
