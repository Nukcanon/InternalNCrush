...
- 기존 2프로세스 피해·재장비·부활·사격, 8인 재접속·서버 재시작·지연 시작, 32명 동시 접속을 Windows에서 통과했습니다. 첫 CI 실행에서 기존 12초 종료 타이머 때문에 32번째 참가 전에 초기 클라이언트가 나갔습니다. 모든 클라이언트가 최신 32인 상태를 확인한 뒤 함께 종료하도록 검사를 수정했습니다. 화면 없는 서버/검사에서는 관절 IK를 건너뛰되 위치·충돌체·시점은 유지합니다. CI의 33개 프로세스 일괄 시작에서는 이후 완료 대기도 시간 초과되어, 각 클라이언트가 접속/첫 스냅샷을 확인한 뒤 다음을 시작하도록 조정했습니다. 먼저 참가한 클라이언트는 종료하지 않으며 최종 32명 동시 연결/최신 스냅샷 검사는 유지합니다. 일괄 시작 폭주를 통과했다고 주장하지 않습니다. 최종 CI 상태는 JSON의 checks 확인.
