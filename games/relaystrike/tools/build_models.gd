...
func run():
	...
	var manifest={"license":"Original Internal N Crush assets; see LICENSE.txt","style":"1.0 stylized human anatomy and grounded equipment","operators":[],"weapons":[],"animations":["idle","walk","run","crouch","crouch_walk","jump","fall","fire","reload","hit","land","death","fall_back","fall_front","fall_left","fall_right","fall_fold"]}
	for role in range(6):
		for team in range(2):
			...
			manifest.operators.append({"role":Rules.CLASSES[role],"name":CharacterVisual.ROLE_NAMES[role],"height_cm":roundi(HumanModel.HEIGHTS[role]*100),"team":team,"scene":path});node.queue_free();await process_frame
