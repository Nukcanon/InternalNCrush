...
func run():
	...
	if server_mode:g.dedicated=true;g.options.map=7;g.host_game();g.start_match()
...
func drive():
	...
	if server_mode:
		...
		if prop.freeze and prop.received and prop.global_position.distance_to(prop.home.origin)>.12 and difference<.18:
			seen_frames+=1
			if seen_frames==20:
				print("PROP_SYNC_OK ",label," error=",difference," displacement=",prop.global_position.distance_to(prop.home.origin)," peers=",g.players.size())
