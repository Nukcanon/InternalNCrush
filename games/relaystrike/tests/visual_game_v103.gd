extends SceneTree
var g:Node
var output="res://../../validation/v103-game-visual"
func _initialize():call_deferred("run")
func capture(label:String):
	if "--gallery" in OS.get_cmdline_user_args() and is_instance_valid(g.ui.stats):g.ui.stats.hide()
	for i in range(6):
		if label=="medic-link":
			g.combat_fx.healing_link(g,g.actors[-1].visual_muzzle(),g.actors[-2].position+Vector3.UP*1.15,-1,-2);g.combat_fx.update_healing(.016)
		await process_frame
...
func run():
	...
	g.set_physics_process(false);g.ui.clear_panel();g.server=true;g.phase="lobby";g.options.map_random=false;g.options.map=13;g.build_world();g.add_player(1,"PLAYER","visual_v103")
	...
	p.role=5;p.primary="m1";p.slot=0;p.team=0;p.protect=0.;g.players[-2].role=0;g.players[-2].slot=0;g.players[-2].team=0;g.players[-2].protect=0.
	...
	g.combat_fx.explosion(a.position+Vector3(1.5,1.,-6.));await create_timer(.14).timeout;await capture("explosion");await create_timer(.7).timeout;await capture("explosion-smoke")
	g.combat_fx.blood_hit(patient.position+Vector3.UP*1.15,Vector3(0,0,-1),30.)
	await create_timer(.22).timeout;await capture("blood-impact")
	for door in g.arena.doors.values():
		a.position=door.global_position+door.basis*Vector3(0,.05,-2.5);a.reset_view(door.rotation.y+PI);a.visual(.1,g.players[1],100.);g.ui.refresh();await capture("door-closed")
		door.opened=true;await create_timer(.9).timeout;await capture("door-open");break
	...
	print("NATIVE_V103_COMPLETE renderer=",RenderingServer.get_video_adapter_name())
