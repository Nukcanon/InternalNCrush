...
func run():
	...
	g.set_physics_process(false);g.ui.clear_panel();g.server=true;g.phase="lobby";g.options.map=13;g.build_world();g.add_player(1,"PLAYER","visual_v102")
	...
	g.options.map=14;g.build_world();await physics_frame;await physics_frame
